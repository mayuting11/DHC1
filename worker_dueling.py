...
class GlobalBuffer:
    ...
    def add(self, data: Tuple):
        '''
        data: actor_id 0, num_agents 1, map_len 2, obs_buf 3, obs_agent_state 4, act_buf 5, rew_buf 6,
        hid_buf 7, td_errors 8, done 9, size 10
        '''
        if data[0] >= 12:
        ...
        with self.lock:
            ...
            for idx, global_idx, local_idx in zip(idxes.tolist(), global_idxes.tolist(), local_idxes.tolist()):
                ...
                for i in range(steps):
                    reward += self.rew_buf[idx + i] * 0.95 ** i
    ...
    def check_done(self):
        for i in range(configs_D3QTP.max_num_agents, configs_D3QTP.max_num_agents + 1):
            ...
            print('The number of tests is :{}'.format(len(l)))
    ...
    def train(self):
        ...
        while not ray.get(self.buffer.check_done.remote()) and self.counter < configs_D3QTP.training_times:
            ...
            for i in range(1, 60000001):
                ...
                td_error = (b_q - (b_reward + (0.95 ** b_steps) * b_q_))
    ...
    def run(self):
        ...
        while True:
            ...
            if (done == False) and (self.env.steps < self.max_episode_length):
                ...
                print('Now the number of agents is {}:'.format(self.env.num_agents))
