...
# def generate_tables(goals_list=configs_D3QTP.goal_list, obstacle_list=configs_D3QTP.obstacle_list,
#                     map_length=configs_D3QTP.init_env_settings[1],
#                     map_width=configs_D3QTP.init_env_settings[2]):
#     delay_dic = configs_D3QTP.delay_dic
#     inf = 2 ** 31 - 1
#     goals_indice = list()
#     for i in goals_list:
...
#
#     q_list = [i for i in goals_indice]
#
#     while q_list:
#         x, y = q_list.pop(0)
#         up = x - 1, y
...
#             dir_dic = relax.relax([up, (x, y)], dir_dic, inf, delay_dic)
...
#             dir_dic = relax.relax([down, (x, y)], dir_dic, inf, delay_dic)
...
#             dir_dic = relax.relax([left, (x, y)], dir_dic, inf, delay_dic)
...
#             dir_dic = relax.relax([right, (x, y)], dir_dic, inf, delay_dic)
#             if not np.all(dir_dic[right] == dir_dic_right):
#                 q_list.append(right)
#     return dir_dic


# def generate_tables_fea(goals_list=configs_D3QTP.goal_list, obstacle_list=configs_D3QTP.obstacle_list,
#                         map_length=configs_D3QTP.init_env_settings[1],
#                         map_width=configs_D3QTP.init_env_settings[2]):
#     delay_dic = configs_D3QTP.delay_dic
#     inf = 2 ** 31 - 1
...
#         if (up[0] > 0) and (list(up) not in obstacle_list):
#             dir_dic_up = copy.deepcopy(dir_dic[up])
#             dir_dic = relax_fea.relax_fea([up, (x, y)], dir_dic, inf, delay_dic)
#             if not np.all(dir_dic[up] == dir_dic_up):
#                 q_list.append(up)
#
#         down = x + 1, y
#         if (down[0] < (map_width - 1)) and (list(down) not in obstacle_list):
#             dir_dic_down = copy.deepcopy(dir_dic[down])
#             dir_dic = relax_fea.relax_fea([down, (x, y)], dir_dic, inf, delay_dic)
#             if not np.all(dir_dic[down] == dir_dic_down):
#                 q_list.append(down)
#
#         left = x, y - 1
#         if (left[1] > 0) and (list(left) not in obstacle_list):
#             dir_dic_left = copy.deepcopy(dir_dic[left])
#             dir_dic = relax_fea.relax_fea([left, (x, y)], dir_dic, inf, delay_dic)
#             if not np.all(dir_dic[left] == dir_dic_left):
#                 q_list.append(left)
#
#         right = x, y + 1
#         if (right[1] < map_length - 1) and (list(right) not in obstacle_list):
#             dir_dic_right = copy.deepcopy(dir_dic[right])
#             dir_dic = relax_fea.relax_fea([right, (x, y)], dir_dic, inf, delay_dic)
#             if not np.all(dir_dic[right] == dir_dic_right):
#                 q_list.append(right)
#     return dir_dic
...
class Environment:
    ...
    def reset(self, num_agents=configs_D3QTP.max_num_agents, map_length=configs_D3QTP.max_map_lenght, map_width=configs_D3QTP.max_map_width):
    ...
    # Get the observation on the fastest waypoint
    # def get_fastest_waypoint(self):
    #     self.fas_map = np.zeros((self.num_agents, self.map_width, self.map_length), dtype=bool)
    #     tables = generate_tables()
    #     # print('tables')
    #     fastest_map = [0 for i in range(self.num_agents)]
    #     for i in range(self.num_agents):
    #         for k in tables[tuple(self.agents_pos[i])]:
    #             if self.agents_deadline[i] >= k[0]:
    #                 fastest_map[i] = k[1]
    #                 break
    #         if fastest_map[i] == 0:
    #             fastest_map[i] = tables[tuple(self.agents_pos[i])][0][1]
    #     for i in range(self.num_agents):
    #         self.fas_map[i][fastest_map[i][0]][fastest_map[i][1]] = 1
    #
    #     self.fas_map = np.pad(self.fas_map,
    ...
    #     tables_fea = generate_tables_fea()
    ...
    #         if not fea_map[j]:
    #             fea_map[j].append(tables_fea[tuple(self.agents_pos[j])][0][1])
    #     for i in range(self.num_agents):
    #         for j in fea_map[i]:
    ...
    def observe(self):
        ...
        obs: shape (num_agents, 11, 2*obs_radius+1, 2*obs_radius+1)
            ...
            # layer 7: fastest map
            # layer 8: feasible map
            layer 7: hazard degree map
            # layer 8-13: one-hot representation of agent's last action
            layer 8: agent's remaining deadline, position, state

        pos: used for calculating communication task
        '''
        obs = np.zeros((self.num_agents, 7, 2 * self.obs_radius + 1, 2 * self.obs_radius + 1))
        ...
        for i, agent_pos in enumerate(
            ...
            # obs[i, 7] = self.feas_map[i][x:x + 2 * self.obs_radius + 1, y:y + 2 * self.obs_radius + 1]
            obs[i, 6] = hazard_map[x:x + 2 * self.obs_radius + 1, y:y + 2 * self.obs_radius + 1]
